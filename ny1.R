...
indi.dist0 <- census(neut, snap = 49)
...
#timeall <- bind_rows(allgens)

timeall <- bind_rows(sample.spabun)

timeall$logsp <- log10(timeall$sp)

saveRDS(timeall,file = paste("/data/zool-avianmacro/jesu3393/papers/Globallocal/divbyyear/ny",y-49,".RDS",sep = ""))
...
save.image(paste('/data/zool-avianmacro/jesu3393/papers/Globallocal/divbyyear/ny',y-49,'.RData', sep = ""))
